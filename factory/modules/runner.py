...
class Run:
    def __init__(self, cfg, adw_id: str, tracer, engineer: str, paths):
        self.cfg = cfg
        self.adw_id = adw_id
        self.tracer = tracer
        ...
        # Both roots come from Paths now rather than from the cwd. `repo_root` was
        # `git_helper.repo_root()`, which answered "whichever repo the process was
        # launched in" — true when the factory lived inside its target, wrong the
        # moment one checkout serves many repos.
        self.paths = paths
        self.repo_root = paths.repo_root           # where every agent is spawned to work
        self.session_dir = ensure_dir(paths.data_dir / "sessions" / adw_id)

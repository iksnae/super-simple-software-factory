...
def _db_path(cfg: SSSFConfig, paths) -> str:
    """The trace db, absolute. Relative in config means "inside the target repo".

    Left relative it would resolve against the cwd, which for a shared factory is
    wherever the operator happened to be standing — one repo's runs landing in
    another repo's db is the exact confusion the two-root split exists to prevent.
    """
    configured = Path(cfg.observability.db)
    return str(configured if configured.is_absolute() else paths.repo_root / configured)


def ensure(cfg: SSSFConfig, paths, adw_id: str | None = None,
           workflow: str = "") -> Run:
    adw_id = adw_id or new_id(8)
    tracer = Tracer(_db_path(cfg, paths),
                    str(paths.data_dir / "sessions" / adw_id / "events.jsonl"))
    run = Run(cfg=cfg, adw_id=adw_id, tracer=tracer, engineer=engineer_name(),
              paths=paths)
    # The workflow name, not the script name. Stock workflows are declared in YAML
    # now, so `Path(sys.argv[0]).stem` would label every run "cli".
    tracer.session_start(adw_id, run.engineer,
                         adw_name=workflow or Path(sys.argv[0]).stem)

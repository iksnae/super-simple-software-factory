...
description: Implement a single Gherkin feature file end-to-end against a target codebase, producing the minimal code needed to satisfy the feature's Background and all its Scenarios. Use this skill when a `.feature` file (Cucumber/Gherkin syntax) defines the work and a working implementation is wanted. Output is whatever code files the feature requires plus a verification command that exercises every scenario. Use a different skill for non-Gherkin specs, multi-feature work (size it with `iteration-plan` first), or for review of existing feature implementations (use `code-quality-review`).
---
...
- Adding tests beyond what the verification command exercises
  (separate skill: `unit-test-coverage`)

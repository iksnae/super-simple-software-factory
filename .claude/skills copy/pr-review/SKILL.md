---
name: pr-review
description: Review one open pull request in this workspace and produce a structured review document containing per-file comments, severity-tagged findings, and an overall verdict. Use this skill when a PR needs structured review feedback. Output is a markdown file at reports/pr-review-<repo>-<n>-<date>.md. Read-only — no comments are posted to the PR itself; the document is the deliverable the operator manually relays. Use `code-quality-review` for a harsher structural pass, or for inspecting a branch that isn't yet a PR.
---
...
- Code changes (`pr-review` is read-only; remediation is the PR
  author's job)
- Merging or rejecting decisions (those are the operator's call)
...
A single markdown file at `reports/pr-review-<repo>-<n>-<YYYY-MM-DD>.md`
with this structure:
...
- The output file exists at `reports/pr-review-<repo>-<n>-<YYYY-MM-DD>.md`

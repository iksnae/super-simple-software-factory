---
name: iteration-plan
description: Break a phase of a milestone plan (or a review report's findings) into one iteration of shippable work items, each estimable in T-shirt size and independently testable. Use this skill after `milestone-planner`, or after a review report, when the operator is ready to commit a phase to delivery. Output is a markdown iteration plan at specs/iterations/<n>-<theme>.md. This workspace uses "iteration" rather than "iteration" deliberately — iterations imply a race; iterations imply thoughtful, repeatable cycles. Slow and steady. We do X sweeps per iteration; we ship every iteration. Do not use this for milestone-level sequencing (use `milestone-planner`).
---
...
## Why "iteration" and not "iteration"

Iterations imply a race against time; iterations imply a repeatable
cycle. Move **thoughtfully over speedily** — slow and
...
cycle. When a doc names "the next iteration", the operator hears:
...
- Cross-iteration dependencies (those live in `milestone-planner`)
...
- **Phase context** — either a phase from a prior `milestone-planner`
  draft or a findings list from a prior review report. Read it with
  `read_file`.

Optional:

- **Iteration number / theme** — operator-supplied. Defaults to next
  available number under `specs/iterations/`.

## Output

A markdown file at `specs/iterations/<n>-<theme>.md`:
...
What the operator shows at iteration review. Concrete:
...
The iteration is done when an external observer can see
...
an inline Mermaid diagram for the decision
tree.

## Failure modes to avoid

- **Iteration vibes.** If you find yourself writing "by next Friday"
...
- **Skills you don't have.** Same rule as `milestone-planner` — if
...
- The output file exists at `specs/iterations/<n>-<theme>.md`

...
import "<module>/internal/termui"   // the repo's shared palette package
...
asserting a string that spans two styled tokens (e.g. `"Khaos Console
...
- **Palette comes from the tokens, never from this file.** Colour is defined
  once in `khaotik-ui/tokens/tokens.json` and the seven-way `StatusTone`
  scale; `DESIGN.md` states the intent. Do not restate hex values in a TUI
  surface and do not introduce a parallel palette (`RULES.md`). No
  gradients, no colours outside the token set.
...
5. Register any new flags (args + flag registry); run the repo's check recipe (`just check`).

## Canonical example

A doctor-style model is the shape to copy: streaming checks (spinner),
ranked rows, focus/expand, re-run, TTY-guarded fallback, themed via a
shared palette package. In this workspace the closest running surfaces
are khaos-tui's five views — read them before building a new one, and
note they are still on the v1 stack.

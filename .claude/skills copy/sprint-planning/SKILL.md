...
description: Plan an ad-hoc, point-based software iteration by selecting backlog items that align with project goals and maximize the likelihood of shipping working code at iteration end
---

# Iteration Planning Skill (Point-Based, Ad-Hoc)

When the user requests iteration planning, operate as a **throughput- and outcome-focused planner**.  
Assume **no dates, no hours, no fixed iteration length**.  
Iterations exist to **ship working code**, not to satisfy a timeline.

## Core Assumptions
- Capacity is measured **only in points**
- Iteration boundaries are flexible and defined by completion
- Velocity is inferred *after the fact* by points shipped per iteration
- Planning optimizes for **finish rate**, not utilization

## Inputs to Request (only if missing)
- Target project or initiative goal
- Prior iteration point throughput (if known)
...
### 2. Iteration Goal Definition
- Define **one concrete shipping outcome**
- The iteration is considered successful only if this outcome is met
- If the goal slips, the iteration is not “done”

### 3. Backlog Selection
- Pull items that **directly advance the iteration goal**
- Prefer:
...
- **Iteration Goal** (1–2 lines, ship-focused)

...
2. Copy that file into the repo under `{{specs_dir}}/`:
   - **List `{{specs_dir}}/` before you pick the name.** A session that plans more than once reuses its `<adw_id>`, so the obvious name may already be taken.
   - Base name: `{{specs_dir}}/<adw_id>_<slug>.md`, where `<adw_id>` is the session directory name inside `context_handoff_dir` (`.../sessions/<adw_id>/context_handoff`) and `<slug>` is two to four kebab-case words naming the work.
   - If a file with that name already exists, use `{{specs_dir}}/<adw_id>_<slug>_v2.md`, then `_v3`, and so on until the name is free. **Never overwrite an existing spec** — the earlier plan is the record of what was asked for then.
   - **Copy it, do not retype it.** One bash call does the whole step:
     `mkdir -p {{specs_dir}} && cp "<context_handoff_dir>/plan.md" "{{specs_dir}}/<adw_id>_<slug>.md"`
...
```json
{
  ...
  "artifacts": ["<context_handoff_dir>/plan.md", "{{specs_dir}}/<adw_id>_<slug>.md"],
